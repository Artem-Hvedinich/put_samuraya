...
export const Status = ({isOwner}: { isOwner: boolean }) => {
    ...
    const activateEditMode = () => {
        if (isOwner) dispatch(statusEditModeAction({statusEditMode: true}))
    }
...
const StatusText = styled.p<{ isOwner: boolean }>`
  padding: 0.1vw 1vw;
  cursor: ${({isOwner}) => isOwner && 'pointer'};

  :hover {
    background-color: ${({isOwner}) => isOwner && 'rgba(128, 128, 128, 0.05)'};
    border-radius: 0.3vw;
  }`